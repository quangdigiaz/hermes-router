...
const ROOT_CA_CN = "Hermes Router MITM Root CA";
...
async function installCertMac(sudoPassword, certPath) {
  ...
  const deleteOld = `security delete-certificate -c "Hermes Router MITM Root CA" /Library/Keychains/System.keychain 2>/dev/null || true`;
...
async function installCertLinux(sudoPassword, certPath) {
  ...
  const config = getLinuxCertConfig();
  const destFile = `${config.dir}/hermes-router-root-ca.crt`;
...
async function uninstallCertLinux(sudoPassword) {
  ...
  const nssResult = updateNssDatabases(null, 'delete');
