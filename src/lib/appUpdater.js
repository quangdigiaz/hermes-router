...
// Collect PIDs of all HermesRouter-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from HermesRouter install dir or wrapper cli.js
        const isAppProcess = lower.includes("hermes-router") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("hermes-router") ||
...
function getDataDir() {
  ...
  if (process.platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "hermes-router");
  }
  return path.join(os.homedir(), ".hermes-router");
}
...
// Resolve npx/HermesRouter binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx HermesRouter` — works regardless of global bin path changes after npm i -g
