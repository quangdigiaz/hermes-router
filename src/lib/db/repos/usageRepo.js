...
// Lazily imported (dynamic) to avoid a static circular dependency:
// notificationBus → allowedModels → localDb → db/index → usageRepo.
let emitNotificationRef = null;
async function getEmitNotification() {
  if (!emitNotificationRef) {
    const mod = await import("../../notificationBus.js");
    emitNotificationRef = mod.emitNotification;
  }
  return emitNotificationRef;
}
...
function aggregateEntryToDay(day, entry) {
  ...
  if (entry.comboName) {
    const comboKey = `${entry.comboName}|${entry.model}|${entry.provider || "unknown"}`;
    addToCounter(day.byCombo, comboKey, { ...vals, meta: { rawModel: entry.model, provider: entry.provider, comboName: entry.comboName } });
  }
}
...
export function trackPendingRequest(model, provider, connectionId, started, error = false) {
  ...
  // Emit dashboard notification for failed requests ([PENDING] END (ERROR))
  if (!started && error) {
    getEmitNotification()
      .then((emit) =>
        emit({
          ...
          status: null,
          message: `Request failed | provider=${provider} | model=${model}`,
          source: "usage",
        })
      )
      .catch(() => {});
...
export async function saveRequestUsage(entry) {
  try {
    ...
    db.transaction(() => {
      ...
      db.run(
        `INSERT INTO usageHistory(timestamp, provider, model, connectionId, apiKey, apiKeyName, comboName, endpoint, promptTokens, completionTokens, cost, status, tokens, meta) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          entry.timestamp, entry.provider || null, entry.model || null,
          entry.connectionId || null, entry.apiKey || null, entry.apiKeyName || null, entry.comboName || null,
          entry.endpoint || null,
  ...
  if (useDailySummary) {
    ...
    for (const dr of dayRows) {
      ...
      for (const [comboKey, combo] of Object.entries(day.byCombo || {})) {
        const comboName = combo.comboName || comboKey.split("|")[0] || "Unknown";
        const rawModel = combo.rawModel || "";
        const provider = combo.provider || "";
        const providerDisplayName = providerNodeNameMap[provider] || provider;
        ...
        stats.byCombo[comboKey].requests += combo.requests || 0;
        stats.byCombo[comboKey].promptTokens += combo.promptTokens || 0;
        stats.byCombo[comboKey].completionTokens += combo.completionTokens || 0;
        stats.byCombo[comboKey].cachedTokens += combo.cachedTokens || 0;
        stats.byCombo[comboKey].cost += combo.cost || 0;
        if (dateKey > (stats.byCombo[comboKey].lastUsed || "")) stats.byCombo[comboKey].lastUsed = dateKey;
      }
    }
    ...
    const filtered = db.all(
      `SELECT timestamp, provider, model, connectionId, apiKey, comboName, endpoint, promptTokens, completionTokens, cost, tokens FROM usageHistory WHERE timestamp >= ?`,
      [cutoff]
    );
    ...
    for (const r of filtered) {
      ...
      if (r.comboName) {
        const comboKey = `${r.comboName}|${r.model}|${r.provider || "unknown"}`;
        if (!stats.byCombo[comboKey]) {
          stats.byCombo[comboKey] = { requests: 0, promptTokens: 0, completionTokens: 0, cachedTokens: 0, cost: 0, comboName: r.comboName, rawModel: r.model, provider: providerDisplayName, lastUsed: r.timestamp };
        }
        const comboEntry = stats.byCombo[comboKey];
        comboEntry.requests++; comboEntry.promptTokens += promptTokens; comboEntry.completionTokens += completionTokens; comboEntry.cachedTokens += cachedTokens; comboEntry.cost += entryCost;
        if (new Date(r.timestamp) > new Date(comboEntry.lastUsed)) comboEntry.lastUsed = r.timestamp;
      }
    }
  }
