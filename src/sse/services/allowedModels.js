...
async function buildAllModelEntries(kindFilter, combos, customModels, modelAliases, isDisabled, activeConnectionByProvider, dbAvailable, options = {}) {
  ...
  // Register Built-in Auto Templates into /v1/models catalog
  if (kindFilter.has(LLM_KIND)) {
    for (const templateName of Object.keys(AUTO_TEMPLATES)) {
      entries.push({ id: templateName, object: "model", owned_by: "auto-router" });
      entries.push({ id: `combo/${templateName}`, object: "model", owned_by: "auto-router" });
    }
  }
