...
export async function handleChat(request, clientRawRequest = null) {
  ...
  // Enforce API key if enabled in settings or if API key is provided
  ...
  if (!trustedInternal) {
    ...
    if (apiKey) {
      ...
      if (!apiKeyInfo && settings.requireApiKey) {
        log.warn("AUTH", "Invalid API key (requireApiKey=true)");
        return errorResponse(HTTP_STATUS.UNAUTHORIZED, "Invalid API key");
      }
    }
    // Check Quota, Rate Limit, Expiration & Model Allowlist
    if (apiKeyInfo) {
      const quotaCheck = await checkApiKeyQuotaAndLimits(apiKeyInfo, modelStr);
      if (!quotaCheck.ok) {
        log.warn("AUTH", `Quota/Limit rejected for key "${apiKeyInfo.name}": ${quotaCheck.error}`);
        return errorResponse(quotaCheck.status || HTTP_STATUS.TOO_MANY_REQUESTS, quotaCheck.error);
      }
    }
  }
...
handleSingleModel: (b, m, opts) => handleSingleModelChat(b, m, clientRawRequest, request, apiKey, apiKeyInfo, opts, modelStr),
  ...
  if (!isAllowed) {
    // Check if model is disabled (turned off in dashboard) vs truly missing
    const disabledByAlias = await getDisabledModels();
    const providerAliases = [provider, provider?.split("/")?.pop()].filter(Boolean);
    const isModelDisabled = providerAliases.some(alias => {
      const disabled = disabledByAlias[alias];
      return Array.isArray(disabled) && (disabled.includes(model) || disabled.includes(resolvedModelStr));
    });
    if (isModelDisabled) {
      log.warn("CHAT", `Model is disabled in provider settings`, { model: resolvedModelStr });
      return errorResponse(HTTP_STATUS.FORBIDDEN, `Model "${resolvedModelStr}" is disabled in provider settings. Enable it in dashboard to use.`);
    }
  ...
  while (true) {
    ...
    if (result.success) return withSelectedConnectionHeader(result.response, credentials.connectionId); // sets X-HermesRouter-Selected-Connection-Id
    ...
    // Daily quota detection: when a 429 error body: when a 429 error body
    ...
    // Emit notification for account unavailability
    if (shouldFallback) {
    ...
    return withSelectedConnectionHeader(result.response, credentials.connectionId); // sets X-HermesRouter-Selected-Connection-Id
  }
}
