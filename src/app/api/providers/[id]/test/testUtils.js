...
async function testOAuthConnection(connection, effectiveProxy = null) {
  ...
  if (config.refreshable && tokenExpired && connection.refreshToken) {
    ...
    if (tokens) {
      ...
      refreshed = true;
      newTokens = tokens;
      // Clear lastError when token refresh succeeds — the connection is healthy again
      newTokens.lastError = null;
      newTokens.lastErrorAt = null;
    } else {
...
async function testApiKeyConnection(connection, effectiveProxy = null) {
  ...
  try {
    switch (connection.provider) {
      ...
      case "opencode": {
        // OpenCode Free uses modelsFetcher (dynamic models) — validate via /models endpoint
        const res = await fetchWithConnectionProxy("https://opencode.ai/zen/v1/models", {
          method: "GET",
          headers: { Authorization: `Bearer ${connection.apiKey}`, "x-opencode-client": "desktop" },
        }, effectiveProxy);
        const valid = res.ok;
        return { valid, error: valid ? null : "Invalid API key" };
      }
      ...
      case "tokenrouter": {
        const res = await fetchWithConnectionProxy("https://api.tokenrouter.com/v1/models", {
          headers: { Authorization: `Bearer ${connection.apiKey}` },
        }, effectiveProxy);
        return { valid: res.ok, error: res.ok ? null : "Invalid API key" };
      }
...
export async function testSingleConnection(id, overrides = null) {
  ...
  // Auto-reactivate: if the connection was previously auth_failed and the test
  // succeeded, reset counters + re-enable any models that were auto-disabled.
  if (connection.testStatus === "auth_failed" && result.valid) {
    emitNotification({
      severity: "info",
      category: "reactivate",
      provider: connection.provider,
      connectionId: id,
      message: `Connection ${id} reactivated after successful test (was auth_failed)`,
      source: "test",
    });
  }
