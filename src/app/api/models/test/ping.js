...
async function pingModelByKindImpl(model, kind, baseUrl = `http://127.0.0.1:${process.env.PORT || UPDATER_CONFIG.appPort}`) {
  ...
  const rawText = await res.text().catch(() => "");
  // Strip SSE terminator that may be appended without newline (forceStream providers)
  const cleanText = rawText?.replace(/\}data: \[DONE\][\s]*$/, "}") || "";
  let parsed = null;
  try {
    parsed = cleanText ? JSON.parse(cleanText) : null;
  } catch (e) {
    console.log("[ping] JSON parse error:", e.message);
  }

  // If parse failed, response might be SSE stream (forceStream providers).
  // Try to extract any data chunk with choices (including reasoning_content).
  if (!parsed?.choices && rawText.includes("data: ")) {
    const lines = rawText.split("\n");
    for (const line of lines) {
      if (!line.startsWith("data: ") || line === "data: [DONE]") continue;
      try {
        const chunk = JSON.parse(line.slice(6));
        if (chunk.choices?.length) { parsed = chunk; break; }
      } catch {}
    }
  }
