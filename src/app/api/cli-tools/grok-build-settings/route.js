...
const hasHermesRouterConfig = (settings) => Boolean(settings?.model?.base_url);
...
export async function POST(request) {
  try {
    ...
    const toml = applyGrokBuildConfig(await readConfigToml(), {
      ...
      apiKey: apiKey || "sk_hermes-router",
      model: selectedModel,
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "Hermes Router model slots removed from Grok Build",
    });
  } catch (error) {
