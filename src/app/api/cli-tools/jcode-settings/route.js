...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-HermesRouter.env");
};
...
const hasHermesRouterConfig = (config) => {
  ...
  if (providers["HermesRouter"]) return true;
...
export async function GET() {
  ...
  const hasHermesRouter = hasHermesRouterConfig(config);
...
export async function POST(request) {
  try {
    ...
    env.JCODE_HermesRouter_API_KEY = apiKey;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "jcode configured successfully. Use: jcode --provider-profile HermesRouter",
      configPath: getConfigPath(),
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    delete config.providers["HermesRouter"];

    await writeConfig(config);

    const env = await readProviderEnv();
    delete env.JCODE_HermesRouter_API_KEY;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "HermesRouter configuration removed from jcode",
    });
  } catch (error) {
