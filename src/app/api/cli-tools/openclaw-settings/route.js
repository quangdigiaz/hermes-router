...
// Check if settings has Hermes Router config
const hasHermesRouterConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["hermes-router"];
};

// Read per-agent models.json and return current model id (without "hermes-router/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["hermes-router"]?.models;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasHermesRouter: hasHermesRouterConfig(settings),
...
const writeAgentModels = async (agentDir, model, baseUrl, apiKey) => {
  ...
  existing.providers["hermes-router"] = {
    baseUrl,
...
// POST - Update Hermes Router settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `hermes-router/${model}`;

    // Remove all old hermes-router/* entries from agents.defaults.models
    for (const k of Object.keys(settings.agents.defaults.models)) {
      if (k.startsWith("hermes-router/")) delete settings.agents.defaults.models[k];
    }
    ...
    // Add fresh Hermes Router models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`hermes-router/${m}`] = {};
    });

    // Remove old Hermes Router model from each agent in agents.list. The
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("hermes-router/")) {
    ...
    // Update models.providers.Hermes Router with all models
    settings.models.providers["hermes-router"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `hermes-router/${agentModel}` };
        return agent;
      });
...
// DELETE - Remove Hermes Router settings only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove Hermes Router from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["hermes-router"];
    ...
    // Remove Hermes Router models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("hermes-router/"));
    ...
    // Reset agents.defaults.model.primary if it uses hermes-router
    if (settings.agents?.defaults?.model?.primary?.startsWith("hermes-router/")) {
    ...
    return NextResponse.json({
      success: true,
      message: "Hermes Router settings removed successfully",
    });
  } catch (error) {
