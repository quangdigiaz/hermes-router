...
const hasHermesRouterConfig = (globalState) => {
  if (!globalState) return false;
  const isOpenAi =
    globalState.actModeApiProvider === "openai" || globalState.planModeApiProvider === "openai";
  const baseUrl = globalState.openAiBaseUrl || "";
  return isOpenAi && (baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("hermes-router"));
};
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasHermesRouter: hasHermesRouterConfig(globalState),
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({ success: true, message: "Hermes Router settings removed from Cline" });
  } catch (error) {
