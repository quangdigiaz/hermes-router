...
const hasHermesRouterConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "Hermes Router");
};

const getHermesRouterEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "Hermes Router") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getHermesRouterEntry(config);
...
// POST - Apply Hermes Router config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing Hermes Router entry or append
    const idx = config.findIndex((e) => e.name === "Hermes Router");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove Hermes Router entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "Hermes Router");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "Hermes Router removed from Copilot config",
    });
  } catch (error) {
