...
function ProviderDetailContent() {
  ...
  const queryConnectionId = searchParams ? searchParams.get("connectionId") : null;
  ...
  // Handle auto-focus and scrolling when queryConnectionId or queryTab is in URL
  useEffect(() => {
    if (!queryConnectionId || connections.length === 0) return;
    const targetIndex = connections.findIndex((c) => String(c.id) === String(queryConnectionId));
    if (targetIndex !== -1) {
      const targetPage = Math.floor(targetIndex / CONNECTIONS_PER_PAGE) + 1;
      setConnectionPage(targetPage);
      setHighlightedConnectionId(String(queryConnectionId));

      const timer = setTimeout(() => {
        const el = document.getElementById(`conn-${queryConnectionId}`);
        if (el) {
  ...
  useEffect(() => {
    if (queryTab === "models") {
      const timer = setTimeout(() => {
        const el = document.getElementById("models-section") || document.querySelector("[data-section='models']");
        if (el) {
  ...
  const handleAddCustomModel = async (modelId, type = "llm", providerAliasOverride = providerStorageAlias, isFree = false) => {
    try {
      const res = await fetch("/api/models/custom", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ providerAlias: providerAliasOverride, id: modelId, type, isFree }),
      });
      if (res.ok) {
        await fetchCustomModels();
          ...
          <div key={conn.id} className="flex min-w-0 items-stretch">
            ...
            <div className="flex-1 min-w-0">
              <ConnectionRow
                ...
                isOAuth={isOAuth}
                isHighlighted={highlightedConnectionId === String(conn.id)}
                isFirst={index === 0}
  ...
  const renderModelsSection = () => {
    if (isCompatible) {
      return (
        <CompatibleModelsSection
          ...
          onAddCustomModel={(modelId, isFree) => handleAddCustomModel(modelId, "llm", providerStorageAlias, isFree)}
    ...
    // Show ALL models (both enabled and disabled) in the main list
    ...
    return (
      <div className="flex flex-col gap-3">
          ...
          return (
            <div className="w-full mt-2">
              <div className="flex items-center justify-between mb-2">
                <p className="text-xs text-text-muted">Suggested free models (≥200k context):</p>
                <button
                  onClick={() => setShowUniversalImportModal(true)}
                  className="text-xs text-primary hover:underline"
                >
                  {translate("Import All")} ({notAdded.length})
                </button>
              </div>
        ...
        <UniversalModelImportModal
          isOpen={showUniversalImportModal}
          onClose={() => setShowUniversalImportModal(false)}
          onImport={async (modelId, name, isFree) => { await handleAddCustomModel(modelId, "llm", providerStorageAlias, isFree); }}
          existingModelIds={[
            ...Object.values(modelAliases),
    ...
    <div className="flex min-w-0 flex-col gap-6 px-1 sm:gap-8 sm:px-0">
      ...
      {connections.length > 0 && models.length > 0 && (
        <ProviderPlayground
          ...
          providerAlias={providerStorageAlias}
          models={models.filter((m) => !disabledModelIds.includes(m.id))}
        />
      )}
