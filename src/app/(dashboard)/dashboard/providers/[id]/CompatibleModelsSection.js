...
function CompatibleModelRow({ modelId, fullModel, copied, onCopy, onDeleteAlias, onTest, testStatus, isTesting, isFree }) {
...
export default function CompatibleModelsSection({ providerStorageAlias, providerDisplayAlias, modelAliases, customModels, copied, onCopy, onDeleteAlias, onAddCustomModel, onDeleteCustomModel, connections, isAnthropic }) {
  ...
  const [newModelIsFree, setNewModelIsFree] = useState(false);
  ...
  const handleAdd = async () => {
    ...
    setAdding(true);
    try {
      await onAddCustomModel(modelId, newModelIsFree);
      setNewModel("");
      setNewModelIsFree(false);
    } catch (error) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <p className="text-sm text-text-muted">
        Add {isAnthropic ? "Anthropic" : "OpenAI"}-compatible models manually or import them from a URL or JSON.
      </p>
      ...
      <div className="flex items-end gap-2 flex-wrap">
        ...
        <Button size="sm" variant="secondary" icon="download" onClick={() => setShowImportModal(true)}>
      ...
      {!canImport && (
        <p className="text-xs text-text-muted">
          {translate("Add a connection to enable importing models from URL.")}
        </p>
      )}

      {allModels.length > 0 && (
        <div className="flex flex-col gap-3">
          {allModels.map(({ id, alias, source, isFree }) => (
            <CompatibleModelRow
      ...
      <UniversalModelImportModal
        ...
        existingModelIds={allModels.map((m) => m.id)}
