...
function modalReducer(state, action) {
  switch (action.type) {
    case "RESET": return { modelId: "", isFree: false, testStatus: null, testError: "", saving: false };
    case "SET_MODEL": return { ...state, modelId: action.value, testStatus: null, testError: "" };
    case "SET_FREE": return { ...state, isFree: action.value };
...
export default function AddCustomModelModal({ isOpen, providerAlias, providerDisplayAlias, onSave, onClose }) {
  const [state, dispatch] = useReducer(modalReducer, { modelId: "", isFree: false, testStatus: null, testError: "", saving: false });
  const { modelId, isFree, testStatus, testError, saving } = state;
      ...
      <div className="flex flex-col gap-4">
        ...
        <label className="flex items-center gap-2 cursor-pointer">
          <input
            type="checkbox"
            checked={isFree}
            onChange={(e) => dispatch({ type: "SET_FREE", value: e.target.checked })}
            className="rounded border-border"
          />
          <span className="text-sm text-text-main">Free model (no cost)</span>
        </label>
