...
export default function ModelTierFilterToolbar({
  ...
  const classification = useMemo(() => {
    ...
    if (providerId === "codex") {
      ...
      return {
        type: "codex",
        tabs: [
          { key: "all", label: `Tất cả (${models.length})`, count: models.length },
    ...
    // Check for Free vs Paid models (TeamoRouter, TokenHarbor, ZenMux, BytePlus, Ollama, etc.)
  ...
  const handleOnlyFree = async () => {
    if (classification.paidModels) {
      const paidIds = classification.paidModels.map((m) => m.id);
      await onDisableModels?.(paidIds);
    }
    if (classification.freeModels) {
      const freeIds = classification.freeModels.map((m) => m.id);
      await onEnableModels?.(freeIds);
    }
  };
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-2.5 rounded-xl border border-primary/20 bg-primary/5 p-2.5 text-xs">
      <div className="flex items-center gap-1.5 flex-wrap">
        <span className="font-semibold text-primary shrink-0">Bộ lọc phân loại:</span>
              ...
              <button
                ...
                className={`rounded-md px-2.5 py-1 text-xs font-semibold transition-colors ${
                  isActive
                    ...
                    : tab.color || "text-text-muted hover:text-text-main"
                }`}
              >
                {tab.label}
              </button>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        {classification.type === "codex" && (
          <Button
            ...
            title="Tắt toàn bộ model Pro (5.6 Sol/Terra/Luna) để tránh lỗi 403 cho tài khoản ChatGPT Plus"
          >
            🛡️ Tắt model Pro (Tránh 403 Plus)
          </Button>
        )}
        ...
        {classification.type === "freepaid" && (
          <Button
            ...
            variant="secondary"
            icon="savings"
            onClick={handleOnlyFree}
            title="Chỉ bật model Free và tắt model tính phí để bảo vệ số dư tài khoản"
          >
            🛡️ Chỉ bật model Free (Tiết kiệm ví)
          </Button>
        )}
      </div>
    </div>
