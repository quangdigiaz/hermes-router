...
export default function ProvidersPage() {
  ...
  const errorProvidersCount = connections
    .filter((c) => c.testStatus === "error" || c.testStatus === "auth_failed" || c.testStatus === "expired")
    .reduce((acc, c) => acc.add(c.provider), new Set()).size;

  const dynamicTierOptions = [
    ...TIER_FILTER_OPTIONS,
    ...(errorProvidersCount > 0
      ? [{ value: "issues", label: `Issues (${errorProvidersCount})`, icon: "warning" }]
      : []),
  ];

  const matchesTier = (info, providerId, authType) => {
  ...
  const freeEntries = Object.entries(FREE_PROVIDERS)
    .filter(([key, info]) => !info.hidden && matchSearch(info.name) && matchesTier(info, key, "no_auth"))
  ...
  const freeTierEntries = Object.entries(FREE_TIER_PROVIDERS)
    .filter(
      ([key, info]) =>
        ...
        matchesTier(info, key, getProviderAuthTypes(info, key)) &&
...
function ProviderCard({ providerId, provider, stats, authType, onToggle, circuitBreaker, onResetCircuit }) {
  ...
  return (
    <Link href={`/dashboard/providers/${providerId}`} className="group min-w-0">
      <Card
        padding="xs"
        className={`h-full hover:bg-black/[0.01] dark:hover:bg-white/[0.01] transition-colors cursor-pointer ${error > 0 ? "border-amber-500/60 dark:border-amber-500/50 bg-amber-50/20 dark:bg-amber-950/10 shadow-xs" : ""} ${allDisabled ? "opacity-50" : ""}`}
      >
          ...
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="flex min-w-0 items-center gap-1.5 text-xs flex-wrap mt-0.5">
                ...
                {provider.badges?.filter(b => b !== "free").slice(0, 2).map(badge => (
