              ...
              <div className="flex flex-col gap-3 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-text-muted">Manual configuration is still available if Hermes Router is deployed on a remote server.</p>
                  </div>
                </div>
          ...
          {!checking && status?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {(status?.config?.provider?.HermesRouter?.options?.baseURL || status?.config?.provider?.["hermes-router"]?.options?.baseURL) && (
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleReset} disabled={!status.hasHermesRouter} loading={restoring}>
...
export default function OpenCodeToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, cloudEnabled, initialStatus, tunnelEnabled, tunnelPublicUrl, tailscaleEnabled, tailscaleUrl }) {
  ...
  const subagentModel = subagentModelOverride ?? (() => {
    ...
    if (model.startsWith("HermesRouter/")) return model.replace("HermesRouter/", "");
    if (model.startsWith("hermes-router/")) return model.replace("hermes-router/", "");
    return "";
  })();
  ...
  const saveModels = async (models) => {
    try {
      const keyToUse = (selectedApiKey && selectedApiKey.trim())
        ? selectedApiKey
        : (!cloudEnabled ? "sk_HermesRouter" : selectedApiKey);
  ...
  const getConfigStatus = () => {
    ...
    if (!status.hasHermesRouter) return "not_configured";
    const url = status.config?.provider?.HermesRouter?.options?.baseURL
      || status.config?.provider?.["hermes-router"]?.options?.baseURL
      || "";
  ...
  const getManualConfigs = () => {
    ...
    return [{
      ...
      content: JSON.stringify({
        ...
        model: `HermesRouter/${activeModelToShow}`,
        agent: {
          explorer: {
