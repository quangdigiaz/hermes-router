          ...
          {!checkingDroid && droidStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {droidStatus?.settings?.customModels?.find(m => m.id?.startsWith("custom:HermesRouter"))?.baseUrl && (
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleResetSettings} disabled={!droidStatus?.hasHermesRouter} loading={restoring}>
...
export default function DroidToolCard({
  ...
  const modelList = modelListOverride ?? (() => {
    ...
    const existingModels = (droidStatus.settings?.customModels || [])
      .filter(m => m.id?.startsWith("custom:HermesRouter"))
      .sort((a, b) => (a.index || 0) - (b.index || 0))
      .map(m => m.model);
    if (existingModels.length > 0) return existingModels;
    const legacy = droidStatus.settings?.customModels?.find(m => m.id === "custom:HermesRouter-0");
  ...
  const getConfigStatus = () => {
    ...
    // Check for any HermesRouter model entry (support multi-model: custom:HermesRouter-0, custom:HermesRouter-1, ...)
    const currentConfig = droidStatus.settings?.customModels?.find(m => m.id?.startsWith("custom:HermesRouter"));
