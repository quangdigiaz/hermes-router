          ...
          {!checkingOpenclaw && openclawStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {openclawStatus?.settings?.models?.providers?.["HermesRouter"]?.baseUrl && (
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleResetSettings} disabled={!openclawStatus?.hasHermesRouter} loading={restoring}>
...
export default function OpenClawToolCard({
  ...
  const getConfigStatus = () => {
    ...
    const currentProvider = openclawStatus.settings?.models?.providers?.["HermesRouter"];
  ...
  if (openclawStatus?.installed && !hasInitializedModel) {
    ...
    const provider = openclawStatus.settings?.models?.providers?.["HermesRouter"];
    if (provider) {
      const primaryModel = openclawStatus.settings?.agents?.defaults?.model?.primary;
      if (primaryModel && !selectedModel) setSelectedModel(primaryModel.replace("HermesRouter/", ""));
  ...
  const getManualConfigs = () => {
    ...
    const settingsContent = {
      agents: {
        defaults: {
          model: {
            primary: `HermesRouter/${selectedModel || "provider/model-id"}`,
          },
        },
      },
      models: {
