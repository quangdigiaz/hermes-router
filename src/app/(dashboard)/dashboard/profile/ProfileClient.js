...
export default function ProfileClient({ initialSettings }) {
  ...
  const updateAutoComboMode = async (mode) => {
    try {
      const res = await fetch("/api/settings", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ autoComboMode: mode }),
      });
      if (res.ok) {
        setSettings(prev => ({ ...prev, autoComboMode: mode }));
      }
    } catch (err) {
      console.error("Failed to update auto combo mode:", err);
    }
  };
  ...
  const handleExportDatabase = async (password) => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `hermes-router-backup-${stamp}.json`;
          ...
          <div className="flex flex-col gap-3 pt-4 border-t border-border">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-3 rounded-lg bg-bg border border-border gap-2">
              <div>
                ...
                <p className="text-xs sm:text-sm text-text-muted font-mono break-all">~/.hermes-router/db/data.sqlite</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 gap-4">
              <div className="flex flex-col gap-2">
                ...
                <Input id="profile-oidc-issuer"
                  placeholder="https://auth.example.com/application/o/hermes-router/"
              ...
              <div className="flex flex-col gap-2">
                ...
                <Input id="profile-oidc-client-id"
                  placeholder="hermes-router-dashboard"
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex items-start sm:items-center justify-between gap-4 pt-4 border-t border-border/50">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs sm:text-sm text-text-muted">
                  How models are selected in combos
                </p>
              </div>
              <select
              ...
              >
                ...
                <option value="auto">Auto (4-factor)</option>
              </select>
            </div>
            ...
            {/* Auto Mode Selector */}
            {settings.comboStrategy === "auto" && (
              <div className="flex items-center justify-between pt-2 border-t border-border/50">
                <div>
                  <p className="font-medium">Auto Mode</p>
                  <p className="text-sm text-text-muted">
                    Scoring priority preset
                  </p>
                </div>
                <select
                  value={settings.autoComboMode || "balanced"}
                  onChange={(e) => updateAutoComboMode(e.target.value)}
                  disabled={loading}
                  className="px-3 py-1.5 rounded-lg border border-border bg-background text-sm"
                >
            ...
            <p className="text-xs text-text-muted italic pt-2 border-t border-border/50">
              ...
              {settings.comboStrategy === "round-robin"
                ...
                : settings.comboStrategy === "cost-optimized"
                ? " Combos try cheapest models first."
                : settings.comboStrategy === "auto"
                ? " Combos use 4-factor scoring (health, cost, latency, model tier)."
