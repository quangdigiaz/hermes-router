...
export default function APIPageClient({ machineId }) {
  ...
  const [editQuotaPeriod, setEditQuotaPeriod] = useState("none");
  ...
  const handleOpenEditKey = async (key) => {
    ...
    setEditTokenLimit(key.tokenLimit !== null && key.tokenLimit !== undefined ? String(key.tokenLimit) : "");
    setEditBudgetLimit(key.budgetLimit !== null && key.budgetLimit !== undefined ? String(key.budgetLimit) : "");
    setEditQuotaPeriod(key.quotaPeriod || "none");
    setEditRpmLimit(key.rpmLimit !== null && key.rpmLimit !== undefined ? String(key.rpmLimit) : "");
  ...
  const handleSaveEditKey = async () => {
    ...
    try {
      const body = {
        ...
        tokenLimit: editTokenLimit ? Number(editTokenLimit) : null,
        budgetLimit: editBudgetLimit ? Number(editBudgetLimit) : null,
        quotaPeriod: editQuotaPeriod || "none",
        rpmLimit: editRpmLimit ? Number(editRpmLimit) : null,
        expiresAt: editExpiresAt ? new Date(`${editExpiresAt}T23:59:59Z`).toISOString() : null,
      };
      ...
      console.log("Error saving key ACL & Quota:", error);
    } finally {
      setEditSaving(false);
    }
  };
              ...
              >
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap items-center gap-3 text-xs text-text-muted mt-1">
                    ...
                    {key.expiresAt && (
                      <span className={`inline-flex items-center gap-1 ${key.isExpired ? "text-red-500 font-semibold" : ""}`}>
                    ...
                    {key.rpmLimit && (
                      <span className="inline-flex items-center gap-1 rounded bg-black/5 dark:bg-white/5 px-1.5 py-0.2 font-mono text-[11px]">
                        RPM: {key.rpmLimit}
                      </span>
                    )}
                    {key.isActive === false && (
                  ...
                  {(key.tokenLimit || key.budgetLimit) && (
                    <div className="mt-2 max-w-md">
                      <div className="flex items-center justify-between text-xs mb-1">
                        <span className="text-text-muted text-[11px]">
                          Quota ({key.quotaPeriod === "daily" ? "Daily" : key.quotaPeriod === "monthly" ? "Monthly" : "Lifetime"}):
                        </span>
                        <span className="font-mono font-medium text-[11px] text-text-main">
                          {key.tokenLimit
                            ? `${(key.usage?.usedTokens || 0).toLocaleString()} / ${key.tokenLimit.toLocaleString()} tokens (${key.tokenPercent || 0}%)`
                            : `$${(key.usage?.usedBudget || 0).toFixed(4)} / $${key.budgetLimit.toFixed(2)} (${key.budgetPercent || 0}%)`}
                        </span>
                      </div>
                      <div className="w-full bg-border/40 h-1.5 rounded-full overflow-hidden">
                        <div
                          className={`h-full transition-all duration-300 ${
                            (key.tokenPercent || key.budgetPercent || 0) >= 90
                              ? "bg-red-500"
                              : (key.tokenPercent || key.budgetPercent || 0) >= 70
                              ? "bg-amber-500"
                              : "bg-primary"
                          }`}
                          style={{ width: `${Math.min(100, Math.max(2, key.tokenPercent || key.budgetPercent || 0))}%` }}
                        />
                      </div>
                    </div>
                  )}
                  ...
                  {(key.allowedProviders || key.allowedCombos || key.allowedKinds || key.allowedModels) && (
                    <div className="flex flex-wrap gap-1 mt-2">
                      ...
                      {key.allowedModels && (
                        <span className="text-[10px] px-1.5 py-0.5 rounded bg-amber-500/10 text-amber-500 dark:bg-amber-500/20">
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="pt-3 border-t border-border-subtle flex flex-col gap-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-semibold text-text-main">Token Limits & Quota</span>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div>
                ...
                <select
                  ...
                  className="w-full rounded-lg border border-border-subtle bg-surface px-3 py-2 text-xs text-text-main outline-none focus:border-primary"
                >
                  <option value="none">Lifetime (Total)</option>
                  <option value="daily">Daily (Reset 00:00)</option>
              ...
              <div>
                ...
                <Input
                  type="number"
                  placeholder="Max req/min (e.g. 15)"
                  value={editRpmLimit}
          ...
          <div className="bg-surface-2 border border-border-subtle rounded-lg p-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-text-muted">
                  Expose your local Hermes Router to the internet. No port forwarding, no static IP needed. Share endpoint URL with your team or use it in Cursor, Cline, and other AI tools from anywhere.
                </p>
              </div>
            </div>
          </div>
