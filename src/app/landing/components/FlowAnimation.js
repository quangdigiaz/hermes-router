...
export default function FlowAnimation() {
  ...
  return (
    <div className="mt-16 w-full max-w-4xl relative h-[360px] hidden md:flex items-center justify-center animate-[float_6s_ease-in-out_infinite]">
      {/* Hermes Router Hub - Center */}
      <div className="relative z-20 w-32 h-32 rounded-full bg-[#0F2847] border-2 border-[#F97316] shadow-[0_0_40px_rgba(249,115,22,0.3)] flex flex-col items-center justify-center gap-1 group cursor-pointer hover:scale-105 transition-transform duration-500">
        <svg viewBox="0 0 32 32" className="w-10 h-10" fill="none">
          <path d="M16 5L22 14L16 27L10 14L16 5Z" fill="#F97316" opacity="0.9"/>
          <path d="M10 14L16 27L10 20L6 14H10Z" fill="#F97316" opacity="0.6"/>
          <path d="M22 14L16 27L22 20L26 14H22Z" fill="#F97316" opacity="0.6"/>
          <circle cx="16" cy="9" r="2" fill="#F97316"/>
        </svg>
        <span className="text-xs font-bold text-white tracking-widest uppercase">
          Hermes
        </span>
        <div className="absolute inset-0 rounded-full border border-[#F97316]/30 animate-ping opacity-20"></div>
      </div>
      ...
      <div className="absolute left-0 top-1/2 -translate-y-1/2 flex flex-col gap-7">
        {CLI_TOOLS.map((tool) => (
          ...
          >
              <div className="w-16 h-16 rounded-2xl bg-[#0F2847] border border-[#1E3A5F] flex items-center justify-center overflow-hidden p-2 hover:border-[#F97316]/50 transition-all hover:scale-105">
              <ProviderIcon
                src={tool.image}
                alt={tool.name}
                size={48}
      ...
      {/* SVG Lines from CLI to Hermes Router */}
      <svg
      ...
      {/* SVG Lines from Hermes Router to Providers */}
      <svg
      ...
      >
        <path
          ...
          fill="none"
          stroke={activeFlow === 0 ? "#F97316" : "rgb(75, 85, 99)"}
        ...
        <path
          ...
          fill="none"
          stroke={activeFlow === 1 ? "#F97316" : "rgb(75, 85, 99)"}
        ...
        <path
          ...
          fill="none"
          stroke={activeFlow === 2 ? "#F97316" : "rgb(75, 85, 99)"}
        ...
        <path
          ...
          fill="none"
          stroke={activeFlow === 3 ? "#F97316" : "rgb(75, 85, 99)"}
        ...
        {PROVIDERS.map((provider, idx) => (
          <div
            ...
            className={`px-4 py-2 rounded-lg ${provider.color} ${provider.textColor} flex items-center justify-center font-bold text-xs shadow-lg hover:scale-110 transition-all cursor-help min-w-[140px] ${
              activeFlow === idx ? "ring-4 ring-[#F97316]/50 scale-110" : ""
            }`}
            title={provider.name}
          >
            {provider.name}
      ...
      {/* Mobile fallback */}
      <div className="md:hidden mt-8 w-full p-4 rounded-lg bg-[#0F2847] border border-[#1E3A5F]">
