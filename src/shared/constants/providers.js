...
function buildProviderEntry(r) {
  ...
  return {
    ...
    // Curation fields (from centralized config, can be overridden by registry)
    ...(() => {
      const curation = getCurationData(r.id);
      return {
        curatedTier: r.curatedTier || curation.curatedTier,
        curatedPriority: r.curatedPriority ?? curation.curatedPriority,
        badges: r.badges || curation.badges,
      };
    })(),
  };
}
