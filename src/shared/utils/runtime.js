 ...
 * @param {string} packageName  npm package name (e.g. "hermes-router")
 * @returns {string|null}
 */
export function updateAndRestartCommand(runtime, packageName) {
  ...
  switch (runtime) {
    case "pm2":
      ...
      const pm2Name = readPm2ProcessName() || "hermes-router";
    ...
    case "screen":
      // Re-launch in a detached screen window named "hermes-router".
      // The user's existing screen session keeps running — they can
      // `screen -r hermes-router` to attach to the new instance.
      return `${installCmd} && screen -dmS hermes-router hermes-router`;
    case "tmux":
      // Re-launch in a detached tmux session named "hermes-router".
      // User can `tmux attach -t hermes-router` to attach to the new instance.
      return `${installCmd} && tmux new-session -d -s hermes-router 'hermes-router'`;
    case "docker":
...
function readSystemdUnitName() {
  ...
  try {
    const exe = process.execPath.split("/").pop() || "hermes-router";
    return `${exe}.service`;
  } catch {
    return null;
  }
