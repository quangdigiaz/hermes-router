...
Triển khai Hermes Router trên VPS hoặc Docker để truy cập từ xa và dùng trong production.
...
| `DATA_DIR` | `~/.hermes-router` | Đường dẫn lưu database và data |
...
Tạo `/etc/nginx/sites-available/hermes-router`:
...
# If NOT using reverse proxy, allow Hermes Router ports
...
```bash
# Check if Hermes Router is running
pm2 status
