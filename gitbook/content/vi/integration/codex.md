...
Tích hợp Hermes Router với OpenAI Codex CLI để định tuyến request API OpenAI qua hệ thống routing thông minh của Hermes Router.
...
Hermes Router cung cấp các model Codex sau:
...
1. Xác minh API key đúng trong Hermes Router dashboard
...
1. Xác minh tên model khớp với cấu hình Hermes Router
2. Kiểm tra kết nối provider OpenAI đang hoạt động trong Hermes Router dashboard
...
Đảm bảo bạn đã cấu hình API key trong Hermes Router cloud dashboard.
