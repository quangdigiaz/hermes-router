...
关于 Hermes Router 的常见问题。

---

## 什么是 Hermes Router?

**Hermes Router 是一款 AI 模型路由工具,能够最大化你的订阅价值并最小化成本。**
...
## 可以自托管 Hermes Router 吗?

**可以!Hermes Router 支持多种部署方式:**
...
**环境变量:**
- `JWT_SECRET` - **生产环境必须修改!**
- `DATA_DIR` - 数据库存储路径(默认:`~/.hermes-router`)
...
**是的,Hermes Router 优先考虑安全和隐私:**

**本地存储:**
- 所有数据存储在本地 `~/.hermes-router`(或自定义 `DATA_DIR`)
- 不会发送数据到 Hermes Router 服务器
...
**破坏性变更:**
- 查看 [CHANGELOG.md](https://github.com/quangdigiaz/hermes-router/blob/main/CHANGELOG.md)
- 大版本更新前备份 `~/.hermes-router`
- 阅读大版本的迁移指南
...
详情见 [CONTRIBUTING.md](https://github.com/quangdigiaz/hermes-router/blob/main/CONTRIBUTING.md)。
