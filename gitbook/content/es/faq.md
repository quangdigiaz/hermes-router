...
Preguntas comunes sobre Hermes Router.

---

## ¿Qué es Hermes Router?

**Hermes Router es un router de modelos de IA que maximiza el valor de tu suscripción y minimiza los costos.**
...
**Hermes Router usa una estrategia de precios de 3 niveles:**
...
## ¿Hermes Router es gratis?

**Sí, Hermes Router en sí es 100% gratis y open source.**
...
**¡Sí! Esta es la característica principal de Hermes Router.**
...
**Hermes Router rastrea la cuota en tiempo real para todos los proveedores:**
...
## ¿Hermes Router funciona con Cursor?

**Sí, pero Cursor requiere un endpoint en la nube.**

**Problema:** Cursor IDE no soporta endpoints en localhost.

**Solución:** Usa el despliegue en la nube de Hermes Router:

```
Cursor Settings → Models → Advanced:
  OpenAI API Base URL: your-cloud-endpoint.com/v1
...
## ¿Puedo auto-hospedar Hermes Router?

**¡Sí! Hermes Router soporta múltiples opciones de despliegue:**
...
- `DATA_DIR` - Ruta de almacenamiento de la base de datos (por defecto: `~/.hermes-router`)
...
**Sí, Hermes Router prioriza la seguridad y privacidad:**

**Almacenamiento local:**
- Todos los datos se almacenan localmente en `~/.hermes-router` (o `DATA_DIR` personalizado)
- No se envían datos a los servidores de Hermes Router
...
### Docker
```bash
docker pull hermes-router:latest
...
**Verificar versión:**
```bash
Hermes Router --version
```

**Cambios disruptivos:**
- Revisa [CHANGELOG.md](https://github.com/quangdigiaz/hermes-router/blob/main/CHANGELOG.md)
- Respalda `~/.hermes-router` antes de actualizaciones mayores
...
1. **Reportar bugs:**
   - [GitHub Issues](https://github.com/quangdigiaz/hermes-router/issues)
   - Incluye logs de error, pasos para reproducir

2. **Solicitar características:**
   - [GitHub Discussions](https://github.com/quangdigiaz/hermes-router/discussions)
   - Describe el caso de uso y los beneficios

3. **Enviar código:**
   ```bash
   # Fork del repo
   git clone https://github.com/YOUR_USERNAME/hermes-router.git
   cd hermes-router
...
Consulta [CONTRIBUTING.md](https://github.com/quangdigiaz/hermes-router/blob/main/CONTRIBUTING.md) para detalles.
