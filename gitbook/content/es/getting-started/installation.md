...
Guía detallada de instalación de Hermes Router con consejos de solución de problemas.
...
Instala Hermes Router globalmente para usar desde cualquier lugar:

```bash
npm install -g hermes-router
```

**Iniciar Hermes Router:**
...
- ✅ Comando simple: `hermes-router`
- ✅ Auto-actualizaciones con `npm update -g hermes-router`
...
```bash
mkdir my-hermes-router
cd my-hermes-router
npm install hermes-router
```
...
3. Se crea el directorio de datos en `~/.hermes-router`
...
**Ubicación por defecto:** `~/.hermes-router`
...
**Error:**
```
Error: EACCES: permission denied, mkdir '/usr/local/lib/node_modules/hermes-router'
```
...
**Problema:** Hermes Router usa demasiada RAM

**Solución: Reinicia el servidor**

```bash
# Detener
pkill -f hermes-router
...
```bash
npm install -g pm2
pm2 start Hermes Router --name hermes-router
pm2 save
```
...
```bash
docker pull hermes-router/hermes-router:latest

docker run -d \
  -p 20128:20128 \
  -e JWT_SECRET="your-secure-secret" \
  -e INITIAL_PASSWORD="your-password" \
  -v hermes-router-data:/root/.hermes-router \
  --name hermes-router \
  hermes-router/hermes-router:latest
```
...
nano ~/.bashrc  # o ~/.zshrc
# Eliminar exports relacionados con hermes-router
```
