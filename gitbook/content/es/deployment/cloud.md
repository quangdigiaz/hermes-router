...
Despliega Hermes Router en VPS o Docker para acceso remoto y uso en producción.
...
| `DATA_DIR` | `~/.hermes-router` | Ruta de almacenamiento de la base de datos |
...
```bash
sudo mkdir -p /var/lib/hermes-router
sudo chown $USER:$USER /var/lib/hermes-router
```
...
```bash
# Ver logs
pm2 logs hermes-router

# Reiniciar aplicación
pm2 restart hermes-router

# Detener aplicación
pm2 stop hermes-router
...
Crea `/etc/nginx/sites-available/hermes-router`:
...
sudo ln -s /etc/nginx/sites-available/Hermes Router /etc/nginx/sites-enabled/
...
# Si NO usas proxy reverso, permite los puertos de Hermes Router
...
tar -czf hermes-router-backup-$(date +%Y%m%d).tar.gz /var/lib/hermes-router

# Respaldo automatizado diario (agregar a crontab)
0 2 * * * tar -czf /backups/hermes-router-$(date +\%Y\%m\%d).tar.gz /var/lib/hermes-router
```
...
# Ver logs
pm2 logs Hermes Router --lines 100
...
pm2 env hermes-router
```

### Nginx 502 Bad Gateway

```bash
# Verificar si Hermes Router está corriendo
pm2 status
...
sudo chown -R $USER:$USER /var/lib/hermes-router
chmod 755 /var/lib/hermes-router
```
