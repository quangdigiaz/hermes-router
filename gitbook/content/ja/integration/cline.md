...
Hermes RouterをCline VSCode拡張機能と統合し、AIリクエストをHermes Routerのインテリジェントルーティングシステム経由でルーティングします。

## 前提条件

- Visual Studio Codeがインストール済み
- VSCodeマーケットプレイスからCline拡張機能がインストール済み
- Hermes Routerがローカルで動作中、またはクラウドエンドポイントが設定済み
...
Base URLをHermes Routerエンドポイントに設定:
...
**手順:**
1. **Base URL** フィールドにHermes Routerエンドポイントを入力
2. 末尾に `/v1` を必ず含める
...
3. キーは `sk-hermes-router-` で始まります

### 5. モデルを選択

1. **Model** ドロップダウンで、次のいずれかを実行:
   - 利用可能なモデルから選択 (Clineが自動検出した場合)
   - Hermes Router設定からモデル名を手動で入力
...
2. チャット入力にメッセージを入力
3. Enterを押して送信
4. ClineはHermes Routerを使用してリクエストを処理

### コード生成

1. Clineにコード生成を依頼: 「Create a React component for a login form」
2. ClineはHermes Routerを使用してコードを生成
...
3. Hermes Router経由でAIによる説明を取得

### ファイル操作

1. Clineにファイルの作成、変更、削除を依頼
2. ClineはHermes Routerを使用してコンテキストを理解し変更を加える
3. 受け入れる前に変更を確認
...
1. Hermes Routerが動作中か確認: `curl http://localhost:20128/health`
...
2. Hermes Routerインスタンスが動作中で正常か確認
3. VSCodeウィンドウをリロードしてみる (Cmd/Ctrl + Shift + P → 「Reload Window」)
4. エラーについてHermes Routerログを確認

## 高度な設定

### クラウドエンドポイントを使用

localhostの代わりにHermes Routerクラウドエンドポイントを使用:

1. Cline設定で、Base URLを設定: `https://your-cloud-endpoint.com`
2. Hermes RouterクラウドダッシュボードでAPIキーが設定されていることを確認
...
Hermes Routerは以下に基づいて最適な利用可能なプロバイダーにリクエストを自動的にルーティング:
- モデル可用性
- プロバイダーヘルスステータス
- コスト最適化
- ロードバランシング
...
プロバイダーが失敗した場合、Hermes Routerは自動的にダッシュボードで設定された代替プロバイダーにフォールバックします。

### 使用量トラッキング

Hermes Routerダッシュボード経由でCline使用量をモニター:
- 総リクエスト数
- トークン使用量
- モデルごとのコスト
- プロバイダー分布
