...
Hermes RouterをOpenAI Codex CLIと統合し、OpenAI APIリクエストをHermes Routerのインテリジェントルーティングシステム経由でルーティングします。
...
Hermes Routerは以下のCodexモデルを提供します:
...
1. Hermes RouterダッシュボードでAPIキーが正しいか確認
...
1. モデル名がHermes Router設定と一致するか確認
2. Hermes RouterダッシュボードでOpenAIプロバイダー接続がアクティブか確認
3. 接続されたプロバイダーでモデルが利用可能か確認

## クラウドエンドポイント

localhostの代わりにHermes Routerクラウドエンドポイントを使用するには:

```bash
export OPENAI_BASE_URL="https://your-cloud-endpoint.com"
```

Hermes RouterクラウドダッシュボードでAPIキーが設定されていることを確認してください。
