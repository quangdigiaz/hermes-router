...
Hermes Routerに関する一般的な質問。

---

## Hermes Routerとは?

**Hermes Routerは、サブスクリプションの価値を最大化し、コストを最小限に抑えるAIモデルルーターです。**
...
**はい、Hermes Router自体は100%無料でオープンソースです。**
...
**Hermes Routerはすべてのプロバイダーのクォータをリアルタイムで追跡します:**
...
## Hermes Routerをセルフホストできますか?

**はい! Hermes Routerは複数のデプロイメントオプションをサポートします:**
...
- `DATA_DIR` - データベース保存パス(デフォルト: `~/.hermes-router`)
...
**はい、Hermes Routerはセキュリティとプライバシーを優先します:**

**ローカルストレージ:**
- すべてのデータは`~/.hermes-router`(またはカスタム`DATA_DIR`)にローカル保存
- Hermes Routerサーバーへのデータ送信なし
- OAuthトークンはJWTで暗号化
...
**Hermes Routerが保存するもの:**
- プロバイダーOAuthトークン(暗号化)
- APIキー(暗号化)
- 使用統計(ローカルのみ)
- コンボ設定

**Hermes Routerが保存しないもの:**
- プロンプトやレスポンス
- 生成したコード
- 個人情報
...
**破壊的変更:**
- [CHANGELOG.md](https://github.com/quangdigiaz/hermes-router/blob/main/CHANGELOG.md)を確認
- メジャー更新前に`~/.hermes-router`をバックアップ
- メジャーバージョンの移行ガイドを確認
...
詳細は[CONTRIBUTING.md](https://github.com/quangdigiaz/hermes-router/blob/main/CONTRIBUTING.md)を参照。
