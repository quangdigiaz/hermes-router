...
リモートアクセスと本番利用のため、VPSまたはDockerにHermes Routerをデプロイ。
...
| `DATA_DIR` | `~/.hermes-router` | データベースとデータの保存パス |
...
`/etc/nginx/sites-available/hermes-router` を作成:
...
# リバースプロキシを使用しない場合、Hermes Routerポートを許可
