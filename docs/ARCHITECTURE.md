# Hermes Router Architecture

_Last updated: 2026-02-06_

## Executive Summary

Hermes Router is a local AI routing gateway and dashboard built on Next.js.
...
flowchart LR
    ...
    subgraph Router[Hermes Router Local Process]
...
- `src/lib/localDb.js`
- file: `${DATA_DIR}/db.json` (or `~/.hermes-router/db.json` when `DATA_DIR` is unset)
- entities: providerConnections, providerNodes, modelAliases, combos, apiKeys, settings, pricing

Usage DB:

- `src/lib/usageDb.js`
- files: `~/.hermes-router/usage.json`, `~/.hermes-router/log.txt`
...
- main state: `${DATA_DIR}/db.json` (or `~/.hermes-router/db.json`)
- usage stats: `~/.hermes-router/usage.json`
- request log lines: `~/.hermes-router/log.txt`
...
flowchart LR
    ...
    subgraph ContainerOrProcess[Hermes Router Runtime]
        Next[Next.js Server\nPORT=20128]
...
1. `usageDb` currently stores under `~/.hermes-router` and does not follow `DATA_DIR`.
...
- Build from source: `cd /root/dev/Hermes Router && npm run build`
- Build Docker image: `cd /root/dev/Hermes Router && docker build -t hermes-router .`
