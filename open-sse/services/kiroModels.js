 ...
 * into hermes-router-shaped variants:
 *
 *   {upstream}                          - base model
 ...
 * API. They are Hermes Router fictions and the `openai-to-kiro` translator strips
...
// notificationBus → allowedModels → kiroModels. Imported at failure time only.
function emitKiroAuthNotification(credentials, status, message) {
  import("@/lib/notificationBus.js")
    .then(({ emitNotification }) =>
      emitNotification({
        ...
        status,
        message: `Kiro ListAvailableModels ${status}: ${message || "credential failure"}`,
        source: "kiro-models",
      })
    )
    .catch(() => {});
...
/**
 * Build the synthetic Hermes Router variant set for a single upstream Kiro model.
 *
 ...
 * into Hermes Router variants (`-thinking`, `-agentic`, `-thinking-agentic`).
 *
...
export async function resolveKiroModels(credentials, options = {}) {
  ...
  try {
    ...
    // Credential-level failures (401/403) feed the dashboard auto-disable pipeline
    // so a login whose token is invalid gets deactivated after repeated hits.
      ...
      if (refreshed?.accessToken) {
        ...
        try {
          ...
          if (err2 && (err2.status === 401 || err2.status === 403)) {
            emitKiroAuthNotification(credentials, err2.status, err2.message);
          }
          return null;
        }
      } else {
