...
async function loadAutoCombo() {
  if (!autoCombo) {
    autoCombo = await import("./autoCombo.js");
  }
  return autoCombo;
}

async function loadLatencyTracker() {
  if (!latencyTracker) {
    latencyTracker = await import("./usage/latencyTracker.js");
  }
...
function cleanExpiredSessions() {
  const now = Date.now();
  for (const [id, session] of sessionProviderMap.entries()) {
    if (now - session.updatedAt > SESSION_TTL_MS) {
      sessionProviderMap.delete(id);
    }
  }
}
...
/**
 * Sort models by input price (cheapest first). Uses promo pricing when available.
...
/**
 * Build candidates and select best model using multi-factor auto scoring.
 * Applies Vision Guard, Context Length Guard, and Session Affinity.
 * @param {string[]} models - Array of model strings (provider/model format)
 * @param {object} [options] - { autoMode, explorationRate, sessionId, promptTokens, hasImages, sessionAffinity }
 * @returns {Promise<string[]>} Models sorted by score (best first)
 */
async function selectAutoModels(models, options = {}) {
  ...
  if (affinityKey) {
    ...
    const validP95s = validCandidates.map((c) => c.p95).filter((p) => p > 0);
    const avgPoolLatency = validP95s.length > 0 ? validP95s.reduce((a, b) => a + b, 0) / validP95s.length : 500;

    if (stickyCandidate && stickyCandidate.health === "CLOSED" && stickyCandidate.p95 <= 2.5 * avgPoolLatency) {
      session.updatedAt = Date.now();
      const rest = validCandidates.filter((c) => c.model !== stickyCandidate.model).map((c) => c.model);
      return [stickyCandidate.model, ...rest];
    } else {
  ...
  const bestCandidates = Object.values(modelGroups).map((group) => {
    const scoredGroup = selectProvider(group, { mode: autoMode, explorationRate: 0 });
    return scoredGroup || group[0];
  });

  bestCandidates.sort((a, b) => (b.score || 0) - (a.score || 0));

  // Exploration 5%
  if (bestCandidates.length > 1 && Math.random() < explorationRate) {
    const randIdx = Math.floor(Math.random() * bestCandidates.length);
    const [explored] = bestCandidates.splice(randIdx, 1);
    bestCandidates.unshift(explored);
  }

  // Record session affinity
  if (sessionId && bestCandidates[0] && bestCandidates[0].health === "CLOSED") {
    sessionProviderMap.set(sessionId, {
      provider: bestCandidates[0].provider,
      model: bestCandidates[0].baseModel,
      p95: bestCandidates[0].p95,
      updatedAt: Date.now(),
    });
  }
...
/**
 * Get rotated model list based on strategy and template
 * @param {string[]} models - Array of model strings
 * @param {string} comboName - Name of the combo
 * @param {string} strategy - "fallback", "round-robin", "cost-optimized", or "auto"
 * @param {number|string} [stickyLimit=1] - Requests per combo model before switching
 * @param {object} [options] - Additional options (autoMode, sessionId, etc.)
 * @returns {Promise<string[]>} Rotated models array
 */
export async function getRotatedModels(models, comboName, strategy, stickyLimit = 1, options = {}) {
  if (!models || models.length <= 1) return models;

  // Check for built-in template
  const template = resolveTemplate(comboName) || resolveTemplate(stripComboPrefix(comboName)) || resolveTemplate(options.template);

  if (template) {
    // Use template.models if defined, fallback to DB models
    let candidatePool = Array.isArray(template.models) && template.models.length > 0
      ? [...template.models]
      : [...models];

    // Apply free filter (includes custom models manually tagged free)
    if (template.filter === "free") {
      const freeCustomMap = await getFreeCustomModelsMap();
      const freeModels = candidatePool.filter((m) => {
    ...
    // Apply family filter (e.g. family/deepseek -> only models with detectModelFamily(model) === "deepseek")
    if (template.family) {
      const familyModels = candidatePool.filter((m) => {
        const slash = m.indexOf("/");
        const model = slash > 0 ? m.slice(slash + 1) : m;
        return detectModelFamily(model) === template.family;
      });
      if (familyModels.length > 0) candidatePool = familyModels;
    }

    // Apply minQuality filter
    if (template.minQuality) {
      const qualified = candidatePool.filter((m) => {
        const slash = m.indexOf("/");
        const model = slash > 0 ? m.slice(slash + 1) : m;
        const bench = getBenchmarkForModel(model);
        if (bench?.quality) return bench.quality / 100 >= template.minQuality;
        return !/mini|budget|tiny/i.test(model);
      });
      if (qualified.length > 0) candidatePool = qualified;
    }

    // Execute strategy
    if (template.strategy === "cost-optimized") {
      return sortModelsByCost(candidatePool);
    }

    return selectAutoModels(candidatePool, {
      ...options,
      autoMode: template.mode,
      sessionAffinity: template.sessionAffinity || false,
    });
  }

  // Default strategies
  if (strategy === "cost-optimized") {
    return sortModelsByCost(models);
  }

  if (strategy === "auto") {
    return selectAutoModels(models, options);
  }
...
export function getComboModelsFromData(modelStr, combosData) {
  ...
  const combo = combos.find(c => c.name === cleanName || c.name === modelStr);
 ...
 */
export async function handleComboChat({ body, models, handleSingleModel, log, comboName, comboStrategy, comboStickyLimit = 1, autoSwitch = true, signal = null, timeoutMs = DEFAULT_COMBO_TARGET_TIMEOUT_MS, queueDepth = null, autoMode = "balanced", explorationRate = 0.05 }) {
  // Apply rotation strategy if enabled
  let rotatedModels = await getRotatedModels(models, comboName, comboStrategy, comboStickyLimit, { autoMode, explorationRate });
  ...
  if (earliestRetryAfter) {
    ...
    emitNotification({
      ...
      model: comboName || rotatedModels[0],
      status,
      message: `All combo models failed: ${msg}`,
      source: "combo",
    });
