...
const FB_STATE_KEY = "__hermes-routerFreebuffState__";
...
// Periodic prune: drop expired session cache + cooldown entries every 5 min
// so long-running servers never accumulate stale state.
if (!fbState._pruneInterval) {
  fbState._pruneInterval = setInterval(() => { pruneSessionState(); }, 5 * 60 * 1000);
  fbState._pruneInterval?.unref?.();
}
...
export class FreebuffExecutor extends BaseExecutor {
  ...
  async execute({ model, body, stream, credentials, signal, log, proxyOptions = null }) {
    ...
    try {
      sessionRef.current = await ensureSession(token, model, proxyOptions);
      // Proactive renewal: if session expires in < 2 min, claim a fresh one
      // to avoid sending requests that will immediately hit 410.
      const EXPIRY_BUFFER_MS = 2 * 60 * 1000;
      if (sessionRef.current?.expiresAt && sessionRef.current.expiresAt - Date.now() < EXPIRY_BUFFER_MS) {
        sessionRef.current = await ensureSession(token, model, proxyOptions, true);
      }
    } catch (error) {
    ...
    const buildBody = () => {
      ...
      if (sessionRef.current?.instanceId) {
        transformed.codebuff_metadata.freebuff_instance_id = sessionRef.current.instanceId;
      }
      return transformed;
    };
