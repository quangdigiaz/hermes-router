...
describe("grokBuildConfig", () => {
  it("creates independent main and per-type subagent model slots", () => {
    ...
    expect(parsed.default).toBe("hermes-router");
    ...
    expect(parsed.subagentMappings).toMatchObject({
      "general-purpose": "hermes-router-general-purpose",
      explore: "hermes-router-explore",
      plan: "grok-4.5",
    });
  ...
  it("is idempotent and updates owned slots without duplicate sections", () => {
    ...
    expect(result.match(/^\[model\.hermes-router\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.hermes-router-general-purpose\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.hermes-router-explore\]$/gm)).toHaveLength(1);
    expect(result.match(/^# hermes-router-prev-subagent-explore/gm)).toHaveLength(1);
  ...
  it("blank override restores previous subagent mapping and removes owned slot", () => {
    ...
    expect(result).not.toContain("[model.hermes-router-explore]");
    expect(parsed.subagentMappings["general-purpose"]).toBe("hermes-router-general-purpose");
  });
  ...
  it("reset restores previous default and all previous subagent mappings", () => {
    ...
    expect(reset).not.toContain("[model.hermes-router-");
    expect(reset).not.toContain("hermes-router-prev-");
  ...
  it("removes mappings that were originally unset", () => {
    ...
    expect(parseGrokBuildConfig(applied).subagentMappings.plan).toBe("hermes-router-plan");
  ...
  it("legacy callers without subagentModels leave existing overrides untouched", () => {
    ...
    expect(parsed.subagentMappings.explore).toBe("hermes-router-explore");
  ...
  it("returns stable slot names only for supported subagent types", () => {
    expect(getGrokSubagentSlot("general-purpose")).toBe("hermes-router-general-purpose");
    expect(getGrokSubagentSlot("explore")).toBe("hermes-router-explore");
    expect(getGrokSubagentSlot("plan")).toBe("hermes-router-plan");
