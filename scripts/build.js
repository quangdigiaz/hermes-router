#!/usr/bin/env node
// Cross-platform production build for the Hermes Router app.
//
...
// The DB lives at DATA_DIR (env or ~/.hermes-router) — outside .next/ so `next build`
...
function backupProductionDb() {
  ...
  const dataDir = process.env.DATA_DIR || dotenv.DATA_DIR || path.join(require("os").homedir(), ".hermes-router");
