...
// Subcommands (`Hermes Router xai video …`) run against an already-running gateway
...
// Self-heal SQLite runtime deps (sql.js + better-sqlite3) into ~/.hermes-router/runtime
...
const PROCESS_IDENTIFIERS = [
  APP_NAME  // Use package name from package.json (e.g. hermes-router)
];
...
function getAppDataDir() {
  return process.platform === "win32"
    ? path.join(process.env.APPDATA || "", "hermes-router")
...
// Kill all Hermes Router processes
    ...
    try {
      ...
      if (platform === "win32") {
        ...
        try {
          ...
          const lines = output.split("\n").slice(1).filter(l => l.trim());
          lines.forEach(line => {
            // Whitelist: real node process running hermes-router/cli.js, or next-server.
            // Avoids killing editors/grep/strace/cursor that just have "hermes-router" in cmdline.
        ...
        try {
          ...
          lines.forEach(line => {
            // Whitelist: real node process running hermes-router/cli.js, or next-server.
            // Avoids killing grep/strace/editors/cursor that incidentally match "hermes-router".
            const cmd = line.toLowerCase();
            const isAppProcess =
...
function startServer(updatePromise) {
      ...
      while (true) {
        ...
        if (choice === "update") {
          ...
          if (process.platform === "darwin") {
            ...
            console.log(`🔔 Hermes Router is running in tray (PID: ${process.pid})`);
            console.log(`   Server: http://${displayHost}:${port}`);
          ...
          console.log(`🔔 Hermes Router is now running in background (PID: ${bgProcess.pid})`);
          console.log(`   Server: http://${displayHost}:${port}`);
          console.log(`\n💡 You can close this terminal. Right-click tray icon to quit.\n`);
  ...
  function tryRestart(code) {
    ...
    if (restartCount >= MAX_RESTARTS) {
      ...
      try {
        const dbPath = path.join(os.homedir(), process.platform === "win32" ? path.join("AppData", "Roaming", "hermes-router", "db.json") : path.join(".hermes-router", "db.json"));
