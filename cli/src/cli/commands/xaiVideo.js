/**
 * `Hermes Router xai video` — generate a Grok Imagine video through the local
 * Hermes Router gateway and save the result as an MP4 file.
 *
...
const HELP = `
Usage: Hermes Router xai video --prompt "..." [options]

Generate a Grok Imagine video via your local Hermes Router gateway
...
Options:
  ...
  --api-key <key>         Hermes Router API key (or env HERMES_ROUTER_API_KEY)
...
function parseArgs(argv) {
  const opts = {
    ...
    port: DEFAULT_PORT,
    host: DEFAULT_HOST,
    apiKey: process.env.HERMES_ROUTER_API_KEY || null,
...
async function run(argv) {
  ...
  try {
    ...
    const connectionId = create.headers["x-hermes-router-connection-id"] || null;
