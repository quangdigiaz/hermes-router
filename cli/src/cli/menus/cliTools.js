...
async function buildCodexHeader() {
  ...
  const { installed, hasHermesRouter, config } = result.data;
...
async function buildDroidHeader() {
  ...
  const { installed, hasHermesRouter, settings } = result.data;
  ...
  // Extract Hermes Router custom model config
  const custom = settings?.customModels?.find(m => m.id === "custom:HermesRouter-0");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenClawHeader() {
  ...
  // Extract Hermes Router provider config
  const provider = settings?.models?.providers?.["hermes-router"];
  const primary = settings?.agents?.defaults?.model?.primary || "";
  const model = primary.startsWith("hermes-router/") ? primary.replace("hermes-router/", "") : (provider?.models?.[0]?.id || "");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenCodeHeader() {
  ...
  const { installed, hasHermesRouter, opencode } = result.data;
...
async function showCliToolsMenu(port, breadcrumb = []) {
  ...
  await showMenuWithBack({
    title: "🔧 CLI Tools",
    breadcrumb,
    headerContent: `Configure CLI tools to use Hermes Router\nEndpoint: ${endpoint}`,
    items: [
      {
        label: "Claude Code",
